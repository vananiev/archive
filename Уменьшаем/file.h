...
int get_from_file(char* file,char *string,int pos=0,unsigned int len=-1)
{
        ...
        in.close();
        string[--i]=0;//последний считанный символ - конец файла
        return 0;
}
//*******************************************************************************************************
...
bool split_str(char *Str,  char* divider,  char* res, int NumOfStr = 2, int LenOfStr = 256)
{
        ...
        return ans; //false - если строка обрезана, не помеща€сь в LenOfStr, или не хватило строк.
}
//*******************************************************************************************************
char* DosToAnsi(unsigned char * s)
{
        for(int i=0;s[i];i++)
                {if(s[i]>=0x80 && s[i]<=0xAF) //јЅ¬-Ёёя,абв-мноп
                        s[i]+=0x40;
                else if(s[i]>=0xE0 && s[i]<=0xEF) //рст-эю€
                        s[i]+=0x10;
                }
        return s;
}
...
{
        for(int i=0;s[i];i++)
                {if(s[i]>=0xC0 && s[i]<=0xEF) //јЅ¬-Ёёя,абв-мноп
                        s[i]-=0x40;
                else if(s[i]<=0xFF && s[i]>=0xF0) //рст-эю€
                        s[i]-=0x10;
                }
        return s;
}
